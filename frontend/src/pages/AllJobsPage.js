...
import { applyForJob } from '../services/authApi';
...
function AllJobsPage({ onNavigate }) {
  ...
  const handleApplyJob = async (jobId) => {
    ...
    try {
      ...
      setAppliedJobs(prevSet => new Set([...prevSet, jobId]));
  ...
  return (
    <>
      ...
      <nav>
        <div className="logo" onClick={() => onNavigate('/user/dashboard')}>
            ...
            {filteredJobs.map((job) => (
              <div key={job.job_id} className="job-card">
                ...
                <div className="job-actions">
                  ...
                  <button 
                    className={`btn ${appliedJobs.has(job.job_id) ? 'btn-applied' : 'btn-save'}`}
                    onClick={() => handleApplyJob(job.job_id)}
                    disabled={appliedJobs.has(job.job_id) || applyingJobId === job.job_id}
                    style={{
                      opacity: appliedJobs.has(job.job_id) ? 0.6 : 1,
                      cursor: appliedJobs.has(job.job_id) ? 'default' : 'pointer'
                    }}
                  >
                    {applyingJobId === job.job_id ? 'Applying...' : appliedJobs.has(job.job_id) ? '✓ Applied' : '✏️ Apply Now'}
                  </button>
                </div>
              </div>
            ))}
